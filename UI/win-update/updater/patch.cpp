...
static int bspatch(const uint8_t *old, int64_t oldsize, uint8_t *newp, int64_t newsize, struct bspatch_stream *stream)
{
	uint8_t buf[8];
...
int ApplyPatch(const wchar_t *patchFile, const wchar_t *targetFile) try {
	uint8_t               header[24];
	int64_t               newsize;
