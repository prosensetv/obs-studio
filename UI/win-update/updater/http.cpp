...
static bool ReadZippedHTTPData(string &responseBuf, z_stream *strm, string &zipBuf, const uint8_t *buffer, DWORD dwOutSize)
{
	do {
		...
		try {
			responseBuf.append(zipBuf.data(), zipBuf.size() - strm->avail_out);
		} catch (...) {
			return false;
		}
...
static bool ReadHTTPData(string &responseBuf, const uint8_t *buffer, DWORD dwOutSize)
{
	try {
		responseBuf.append((const char *)buffer, dwOutSize);
	} catch (...) {
		return false;
	}
	return true;
                  ...
                  string &       responseBuf)
{
	...
	bool bResults = !!WinHttpSendRequest(hRequest, extraHeaders, extraHeaders ? -1 : 0, (void *)data, dataLen, dataLen, 0);
	...
	*responseCode = wcstoul(statusCode, nullptr, 10);
	...
	do {
		...
		if (!WinHttpReadData(hRequest, (void *)buffer, dwSize, &dwOutSize)) {
			*responseCode = -9;
			return false;
		} 
		...
		if (gzip) {
			if (!ReadZippedHTTPData(responseBuf, strm, zipBuf, buffer, dwOutSize)) {
				*responseCode = -6;
				return false;
			}
		} else {
			if (!ReadHTTPData(responseBuf, buffer, dwOutSize)) {
				*responseCode = -6;
				return false;
			}
		}
...
static bool ReadHTTPZippedFile(z_stream *strm, HANDLE updateFile, string &zipBuf, const uint8_t *buffer, DWORD dwOutSize, int *responseCode)
{
	do {
		strm->avail_in = dwOutSize;
		strm->next_in  = buffer;
...
static bool ReadHTTPFile(HANDLE updateFile, const uint8_t *buffer, DWORD dwOutSize, int *responseCode)
{
	DWORD written;
	if (!WriteFile(updateFile, buffer, dwOutSize, &written, nullptr)) {
		*responseCode = -12;
		return false;
	}

	if (written != dwOutSize) {
		*responseCode = -13;
		return false;
	}
	...
	do {
		...
		if (!WinHttpReadData(hRequest, (void *)buffer, dwSize, &dwOutSize)) {
			*responseCode = -9;
			...
			if (gzip) {
				if (!ReadHTTPZippedFile(strm, updateFile, zipBuf, buffer, dwOutSize, responseCode))
					return false;
			} else {
				if (!ReadHTTPFile(updateFile, buffer, dwOutSize, responseCode))
					return false;
			}
